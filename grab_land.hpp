...
class grabland: public eosio::contract {
  public:
     grabland( account_name self );
  ...
  private:
     ...
     uint64_t get_add_time( const game_status & game, uint64_t land_amount );
     uint64_t calc_range(uint64_t st, uint64_t ed);
     void handle_airdrop(account_name buyer, const game_status& game, const uint64_t& land_amount);
     ...
     uint64_t get_land_amount(asset quant, uint64_t supply);

     global_index _global;
     game_index _games;

     const symbol_type LAND = S(0, LAND);
     const symbol_type HXC = S(4, HXC);
     ...
     const uint64_t HXC_COIN_REWARD[6] = {1, 2, 2, 2, 2, 2};
     const uint64_t REMAIN_TOKEN = 100000000 - 62000000;
     const uint64_t DAY = 86400000000;

     const account_name BANCOR_ACCOUNT = N(bctphuoxinio);
     const account_name OPEX_ACCOUNT = N(opexhuoxinio);
     const account_name HXC_MINE_ACCOUNT = N(minehuoxinio);
