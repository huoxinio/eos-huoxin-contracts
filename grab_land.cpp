...
#include "grab_land.hpp"

#define EOS_SYMBOL S(4, EOS)

#define TOKEN_CONTRACT N(eosio.token)
#define HXC_CONTRACT N(coinhuoxinio)
...
grabland::grabland( account_name self )
         :contract(self),
         _global(_self, _self),
         _games(_self, _self)
{
...
void grabland::transfer( account_name from,
            account_name to,
            asset        quantity,
            string       memo )
{
    ...
    eosio_assert( (itr->is_start == 1) || (from == N(ismehuoxinio) ), "Not start yet!");
...
uint64_t grabland::get_land_amount(asset quant, uint64_t supply)
{
...
void grabland::buy( account_name buyer, asset quant, account_name inviter)
{
    require_auth( buyer );
    ...
    uint64_t land_amount = get_land_amount(quant, game.supply);

    eosio_assert(land_amount >= 1, "need at least buy one key");
    eosio_assert( asset( calc_range(game.supply + 1, game.supply + land_amount) / 10000, EOS_SYMBOL ) <= quant, "big error! can not buy these land amount" );
    ...
    // 53%  ========> dividend
    asset dividend_fee = asset(quant.amount * 53 / 100, EOS_SYMBOL ); 
    // 4%   ========> airdrop
    asset airdrop_fee = asset(quant.amount * 4 / 100, EOS_SYMBOL ); 
    // 30%  ========> bancor + inviter
    asset bancor_fee = asset( 0, EOS_SYMBOL );

    asset inviter_fee = asset( 0, EOS_SYMBOL );
    if ( inviter != buyer ) {
        // for user sale more than 50w land, reward 10% 
        if (inviter_cnt >= 500000) {
            bancor_fee = asset(quant.amount * 20 / 100, EOS_SYMBOL ); 
            inviter_fee = asset(quant.amount * 10 / 100, EOS_SYMBOL ); 
        } else  {
            bancor_fee = asset(quant.amount * 25 / 100, EOS_SYMBOL ); 
            inviter_fee = asset(quant.amount * 5 / 100, EOS_SYMBOL ); 
        }
    } else {
        bancor_fee = asset(quant.amount * 30 / 100, EOS_SYMBOL ); 
    }

    // 5% ========> opex 
    asset opex_fee = asset(quant.amount * 5 / 100, EOS_SYMBOL ); 
    // 8%   ========> lottery
    asset last_reward_fee = asset(quant.amount * 8 / 100, EOS_SYMBOL ); 
    ...
    if (inviter_fee.amount > 0) {
        ...
        invitertable.modify(*inviter_itr, 0, [&]( auto& ivter ) {
                ...
                ivter.sale_amount += land_amount;
        });
    }

    handle_airdrop( buyer, game, land_amount );
    
    // calc mined token 
    uint64_t mined_token = land_amount;
    mined_token = std::min(global.get_rest_hxc(), mined_token);

    if (mined_token > 0) {
        asset tokenout = asset(safe_mul(mined_token,10000), HXC);
        action(
          permission_level{HXC_MINE_ACCOUNT, N(active)},
          HXC_CONTRACT, N(transfer),
          make_tuple(HXC_MINE_ACCOUNT, buyer, tokenout, std::string("mined token")))
          .send();
    }

    // calc added life
    uint64_t add_time = get_add_time( game, land_amount );

    // update user dividend infomation
    usertable.modify(user, 0, [&]( auto& usr ) {
        // skip the previous dividend
        usr.mask += calc_range(game.supply, game.supply+land_amount-1);
        usr.balance += land_amount;
    });

    // update game status
    _games.modify(game, 0, [&]( auto& gm ) {
        gm.supply += land_amount;
        uint64_t c_time = current_time();
...
void grabland::withdraw( account_name player )
{
    require_auth( player );
    ...
    asset payout = asset( safe_mul(total, 53) / 100 / 10000, EOS_SYMBOL );
...
void grabland::claim( account_name player ) {
        ...
        airdroptable.modify(*itr, 0, [&]( auto& aim ) {
            ...
            for (uint64_t i = 0; i < aim.rewards.size(); i ++) {
                if (aim.rewards[i].player == player && aim.rewards[i].status == 1) {
                    ...
                    if ( id < 160) {
                        tokenout += asset( 2000000, EOS_SYMBOL );
                    } else if ( id < 240 ) {
                        tokenout += asset( 4000000, EOS_SYMBOL );
                    } else if ( id < 280  ) {
                        tokenout += asset( 8000000, EOS_SYMBOL );
                    } else if ( id < 300 ) {
                        tokenout += asset( 16000000, EOS_SYMBOL );
                    } else if ( id < 310 ) {
                        tokenout += asset( 32000000, EOS_SYMBOL );
                    } else {
                        uint64_t st = id * 100000 + 1;
                        uint64_t ed = (id+1) * 100000;
                        uint64_t total = calc_range(st, ed);
                        tokenout += asset( safe_mul(total, 4) / 100 / 10000, EOS_SYMBOL);
                    }
...
uint64_t grabland::get_stage(uint64_t point)
{
    uint64_t idx = 0;
...
uint64_t grabland::get_add_time(const game_status & game, uint64_t land_amount ) {
    uint64_t c_stage = get_stage( game.supply + 1 );
    
    uint64_t res = safe_mul( STAGE_TIME[ c_stage ], land_amount );
    return res;
}

uint64_t grabland::safe_mul( uint64_t a, uint64_t b ) {
    uint128_t tmp = (uint128_t)a * (uint128_t)b;
    eosio_assert( tmp <= max_amount, "multiplication overflow" );
    return tmp;
}

void grabland::handle_airdrop(account_name buyer, const game_status& game,  const uint64_t&  land_amount) {
    uint64_t start = game.supply;

    uint64_t end = game.supply + land_amount;
...
uint64_t grabland::calc_range(uint64_t st, uint64_t ed)
{
    if (st > ed) {
        return 0;
    }
...
void grabland::finish()
{
    ...
    action(
      permission_level{_self, N(active)},
      BANCOR_ACCOUNT, N(flowtoquote),
      make_tuple(_self, asset(into_bancor, EOS_SYMBOL), std::string("flow to bancor pool")))
      .send();
    // set cleaned
    _games.modify(game, 0, [&]( auto& gm ) {
            gm.cleaned = 1;
...
// EOSIO_ABI_PRO(grabland, (transfer)(withdraw)(finish)(start))

// generate .abi file
EOSIO_ABI(grabland, (transfer)(withdraw)(finish)(start))
